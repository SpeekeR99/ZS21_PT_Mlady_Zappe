...
/**
 * Compares two <code>src.GraphNode</code>s based on their angle and distance
 * from another (reference) <code>src.GraphNode</code>. The reference node has to be set before comparing!
 * <br><br>
 * src.Main idea: all points that have similar angle from the reference node are close to each other.
